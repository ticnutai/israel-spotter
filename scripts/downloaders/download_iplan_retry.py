"""
Retry failed iPlan layer downloads with improved query method.
Uses resultOffset pagination instead of objectid-based pagination.
"""
...
def query_layer_simple(url, where="1=1", extent=None, out_sr=2039):
    """Simple query without objectid pagination - use resultOffset instead."""
    ...
    while True:
        ...
        if data.get("error"):
            # If resultOffset not supported, try without it
            if offset == 0:
...
RETRY_LAYERS = [
    ...
    # Gas pipelines
    {"name": "gas_pipelines", "url": "https://ags.iplan.gov.il/arcgisiplan/rest/services/PlanningPublic/gaz_compilation/MapServer/2", "extent": WIDE_EXTENT, "desc": "גז טבעי – תוואי הולכה"},
    {"name": "gas_fuel_pipes", "url": "https://ags.iplan.gov.il/arcgisiplan/rest/services/PlanningPublic/gaz_compilation/MapServer/3", "extent": WIDE_EXTENT, "desc": "גז טבעי – צנרות דלק"},
    ...
    # TAMA 35 roads & rails
    {"name": "tama35_roads", "url": "https://ags.iplan.gov.il/arcgisiplan/rest/services/PlanningPublic/Tama_35_1/MapServer/15", "extent": WIDE_EXTENT, "desc": "תמ\"א 35 – דרכים"},
    {"name": "tama35_train", "url": "https://ags.iplan.gov.il/arcgisiplan/rest/services/PlanningPublic/Tama_35_1/MapServer/13", "extent": WIDE_EXTENT, "desc": "תמ\"א 35 – מסילות ברזל"},

    # TAMA 1 gas
    {"name": "tama1_gas_pipeline", "url": "https://ags.iplan.gov.il/arcgisiplan/rest/services/PlanningPublic/TAMA_1/MapServer/17", "extent": WIDE_EXTENT, "desc": "תמ\"א 1 – קו גז טבעי"},
    ...
    # Xplan - try with spatial filter instead
    {"name": "xplan_points", "url": "https://ags.iplan.gov.il/arcgisiplan/rest/services/PlanningPublic/Xplan/MapServer/0", "extent": EXTENT, "desc": "Xplan – ישויות נקודתיות (spatial)", "where": "1=1"},
    {"name": "xplan_lines", "url": "https://ags.iplan.gov.il/arcgisiplan/rest/services/PlanningPublic/Xplan/MapServer/2", "extent": EXTENT, "desc": "Xplan – ישויות קוויות (spatial)", "where": "1=1"},
    {"name": "xplan_polygons", "url": "https://ags.iplan.gov.il/arcgisiplan/rest/services/PlanningPublic/Xplan/MapServer/3", "extent": EXTENT, "desc": "Xplan – ישויות פוליגונליות (spatial)", "where": "1=1"},
    {"name": "xplan_land_use", "url": "https://ags.iplan.gov.il/arcgisiplan/rest/services/PlanningPublic/Xplan/MapServer/4", "extent": EXTENT, "desc": "Xplan – יעודי קרקע (spatial)", "where": "1=1"},
]
...
def main():
    ...
    print(f"Retrying {len(RETRY_LAYERS)} failed layers...")
    print("=" * 60)

    for idx, layer in enumerate(RETRY_LAYERS):
        name = layer["name"]
        out_path = os.path.join(OUTPUT_DIR, f"{name}.geojson")

        # Skip if already downloaded successfully (non-empty)
        if os.path.exists(out_path):
            with open(out_path, "r", encoding="utf-8") as f:
                existing = json.load(f)
            if len(existing.get("features", [])) > 0:
                print(f"[{idx+1}/{len(RETRY_LAYERS)}] SKIP {name} (already has {len(existing['features'])} features)")
                results.append({"name": name, "status": "skip", "features": len(existing["features"])})
                continue

        print(f"[{idx+1}/{len(RETRY_LAYERS)}] {layer['desc']}")
        where = layer.get("where", "1=1")
        features = query_layer_simple(layer["url"], where=where, extent=layer.get("extent"))
    ...
    print(f"\n{'='*60}")
    print(f"Retry results: {ok} downloaded, {skip} skipped, {empty} empty")

    summary_path = os.path.join(OUTPUT_DIR, "_retry_summary.json")
