...
// ─── Selector-based hooks for granular subscriptions ─────────────────────────

let _nextId = 1;

// Stable action references (created once, never change)
const actions = {
  addLayer: (layer: Omit<MapLayer, "id" | "order">) => {
    const id = `layer-${Date.now()}-${_nextId++}`;
    setState((prev) => ({
      ...prev,
      layers: [...prev.layers, { ...layer, id, order: prev.layers.length }],
    }));
    return id;
  },
  removeLayer: (id: string) => {
    setState((prev) => ({
      ...prev,
      layers: prev.layers.filter((l) => l.id !== id).map((l, i) => ({ ...l, order: i })),
    }));
  },
  updateLayer: (id: string, updates: Partial<MapLayer>) => {
    setState((prev) => ({
      ...prev,
  ...
  setFillOpacity: (id: string, fillOpacity: number) => {
    setState((prev) => ({
      ...prev,
      layers: prev.layers.map((l) =>
  ...
  reorderLayers: (fromIndex: number, toIndex: number) => {
    setState((prev) => {
      ...
      return { ...prev, layers: layers.map((l, i) => ({ ...l, order: i })) };
    });
  },
  duplicateLayer: (id: string) => {
    setState((prev) => {
      ...
      const copy: MapLayer = { ...source, id: newId, name: source.name + " (עותק)", order: prev.layers.length, locked: false };
  ...
  paintParcel: (gush: number, helka: number, color: string, fillColor?: string, fillOpacity?: number, label?: string) => {
    setState((prev) => {
      const existing = prev.paintedParcels.findIndex((p) => p.gush === gush && p.helka === helka);
      const entry: PaintedParcel = { gush, helka, color, fillColor: fillColor ?? color, fillOpacity: fillOpacity ?? 0.35, label };
      const parcels = [...prev.paintedParcels];
      if (existing >= 0) parcels[existing] = entry;
      else parcels.push(entry);
      return { ...prev, paintedParcels: parcels };
    });
  },
  unpaintParcel: (gush: number, helka: number) => {
    setState((prev) => ({
      ...prev,
      paintedParcels: prev.paintedParcels.filter((p) => !(p.gush === gush && p.helka === helka)),
    }));
  },
  clearPaintedParcels: () => {
    setState((prev) => ({ ...prev, paintedParcels: [] }));
  },
  updatePaintedParcel: (gush: number, helka: number, updates: Partial<PaintedParcel>) => {
    setState((prev) => ({
      ...prev,
  ...
  updateLabelSettings: (updates: Partial<ParcelLabelSettings>) => {
    setState((prev) => ({
      ...prev,
      labelSettings: { ...prev.labelSettings, ...updates },
    }));
  },
  updateBorderSettings: (updates: Partial<ParcelBorderSettings>) => {
    setState((prev) => ({
      ...prev,
...
/** Full store hook – use sparingly, prefer selector hooks below */
export function useLayerStore() {
  const state = useSyncExternalStore(subscribe, getSnapshot);
  return { ...state, ...actions };
}

/** Subscribe only to layers array */
export function useStoreLayers() {
  return useSyncExternalStore(subscribe, () => getSnapshot().layers);
}

/** Subscribe only to painted parcels */
export function useStorePaintedParcels() {
  return useSyncExternalStore(subscribe, () => getSnapshot().paintedParcels);
}

/** Subscribe only to label settings */
export function useStoreLabelSettings() {
  return useSyncExternalStore(subscribe, () => getSnapshot().labelSettings);
}

/** Subscribe only to border settings */
export function useStoreBorderSettings() {
  return useSyncExternalStore(subscribe, () => getSnapshot().borderSettings);
}
